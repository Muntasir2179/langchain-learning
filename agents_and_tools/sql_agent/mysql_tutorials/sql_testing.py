...
class MySQLDatabase:
    ...
    def insert_data(
        ...
        try:
            ...
            if conn.is_connected():
                cursor.close()
                conn.close()
            print(f"Data inserted successfully into the table with user id --{current_user_id}--")
        except Exception as e:
    ...
    def search_data(self, user_id):
        ...
        result = None
        try:
            search_query = f"SELECT * FROM {self.table_name} WHERE user_id = %s"   # sql query for searching data
            cursor.execute(search_query, (user_id,))
            result = self.__format_search_result(cursor.fetchone())
            if conn.is_connected():
                cursor.close()
                conn.close()
        except Exception as e:
    ...
    def update_data(self, user_id, phone_number: str = None, person_name: str = None, age: int = None, appointment_date: str = None, appointment_time: str = None):
        try:
            ...
            if result:  # when there is a data exist to update
                ...
                if update_fields:
                    update_query = f"UPDATE {self.table_name} SET {', '.join(update_fields)} WHERE user_id = %s"
                    update_values.append(user_id)  # Add the phone_number to the end for the WHERE clause
                    cursor.execute(update_query, tuple(update_values))
                    conn.commit()
                    
                    print(f"Data for user id --{user_id}-- updated successfully.")
                else:
                    print("No new information provided to update.")
            else:
                print(f"No data found for user id {user_id}. Update not performed.")
        except Exception as e:
    ...
    def delete_data(self, user_id:int):
        try:
            conn, cursor = self.__get_connection_cursor()
            result = self.__format_search_result(self.search_data(user_id=user_id))
            print(result)
            if result:
                delete_query = f"DELETE FROM {self.table_name} WHERE user_id = %s"    # sql query for deleting data from table
                cursor.execute(delete_query, (user_id,))
                conn.commit()
                print(f"Data deleted from the table with user id --{user_id}--\n")
            else:
                print(f"Data with this user id --{user_id}-- does not exists.")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # db.insert_data(phone_number="09876543210", person_name="Jane Smith", appointment_date="2024-10-02", appointment_time="11:00:00")

    # db.delete_data(user_id=3)
    
    # db.update_data(phone_number="09876543210")
    # db.update_data(user_id=2, phone_number="10", person_name="Muntasir Ahmed")

    # result = db.search_data(user_id=2)
    # print(result)
